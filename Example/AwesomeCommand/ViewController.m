...
- (void)testRequestCMD {
    ...
    id<AwesomeCallback> callback = [[AwesomeCallbackObject alloc] init];
    RACSignal *signal = [requestCMD createSignal];
    RACSubject *subject = [RACSubject subject];
    [requestCMD subject:subject];
    //第一种方式通过callback回调获得回调值
    id<AwesomeCancelable> cancelObject_one = [requestCMD executeWithCallback:callback];
...
- (void)testMutilThread {
    ...
    multiCMD.excuteBlock = (id) ^(id<AwesomeResult> result) {
        ticketSellor(index);
...
- (void)testForCombine {
  ...
  @weakify(self);
  self.firstCMD.excuteBlock = (id) ^ (id<AwesomeResult> result) {
    // Example
    @strongify(self);
